...
RegisterNetEvent('qb-customs:server:attemptPurchase', function(type, upgradeLevel, vehiclename)
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
    local moneyType = Config.MoneyType
    local balance = Player.Functions.GetMoney(moneyType)
	local vehiclePrice = vehicleprices[vehiclename]['price']
    ...
    if type == "repair" then
        ...
        if balance >= repairCost then
            Player.Functions.RemoveMoney(moneyType, repairCost, "Tamir")
...
--	exports['qb-management']:AddMoney("mechanic", repairCost)
    ...
    elseif type == "performance" or type == "turbo" then
        if balance >= vehicleCustomisationPrices[type].prices[upgradeLevel] then
            TriggerClientEvent('qb-customs:client:purchaseSuccessful', source)
            Player.Functions.RemoveMoney(moneyType, vehiclePrice*vehicleCustomisationPrices[type].prices[upgradeLevel]//75, "Modifiye")
	--exports['qb-management']:AddMoney("mechanic", vehicleCustomisationPrices[type].prices[upgradeLevel])
        ...
        if balance >= vehicleCustomisationPrices[type].price then
            TriggerClientEvent('qb-customs:client:purchaseSuccessful', source)
            Player.Functions.RemoveMoney(moneyType, vehiclePrice*vehicleCustomisationPrices[type].price//600, "Modifiye")
...
RegisterNetEvent("qb-customs:server:updateVehicle", function(car, plate)
--    if IsVehicleOwned(plate) then
        MySQL.update('UPDATE player_vehicles SET mods = ? WHERE plate = ?', {json.encode(car), plate})
 --   end
end)
