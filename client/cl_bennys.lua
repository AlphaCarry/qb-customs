...
local function saveVehicle()
    ...
    TriggerServerEvent('qb-customs:server:updateVehicle', myCar, plate)
end
...
--print(vehiclename2)
    ...
    TriggerServerEvent("qb-customs:server:attemptPurchase", type, upgradeLevel, vehiclename2)
...
--    for i, spot in ipairs(data.zones) do
--	print(data.zones)
	local dist = #(pos - vector3(-457.82, 6044.13, 30.36))
	local dist2 = #(pos - vector3(394.86, -940.38, 21.21))
	local dist3 = #(pos - vector3(-381.42, -361.15, 24.05)) 
 -- if player.job.name == 'police' then
	 if dist < 30 and IsPedInAnyVehicle(ped,false) then
	DrawMarker(25, -457.82, 6044.13, 30.36, 0, 0, 0, 0, 0, 0, 5.0, 5.0, 5.0, 255, 0, 0, 255, 0, 0, 0, 0, 0, 0, 0)
	--DrawText3Ds(-457.82, 6044.13, 31.36, "[~g~E~w~] - Mekanik")
	end
	if dist2 < 30 and IsPedInAnyVehicle(ped,false) then
	DrawMarker(25, 394.86, -940.38, 21.21, 0, 0, 0, 0, 0, 0, 5.0, 5.0, 5.0, 255, 0, 0, 255, 0, 0, 0, 0, 0, 0, 0)
	--DrawText3Ds(394.86, -940.38, 22.21, "[~g~E~w~] - Mekanik")
	end
	if dist3 < 30 and IsPedInAnyVehicle(ped,false) then
	DrawMarker(25, -381.42, -361.15, 23.80, 0, 0, 0, 0, 0, 0, 5.0, 5.0, 5.0, 255, 0, 0, 255, 0, 0, 0, 0, 0, 0, 0)
	--DrawText3Ds(394.86, -940.38, 22.21, "[~g~E~w~] - Mekanik")
	end
 -- end
--  end
...
RegisterNetEvent("qb-customs:client:purchaseSuccessful", function()
    ...
    QBCore.Functions.Notify("Satın alma başarılı")
end)

RegisterNetEvent("qb-customs:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Yeterli paran yok", "info")
end)
